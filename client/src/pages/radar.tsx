...
export default function Radar({ user }: RadarProps) {
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col">
      ...
      {/* Admin-only QBOT button for non-header users */}
      {!user.isAdmin && (
        <div className="absolute top-4 right-4 z-[1002] flex items-center space-x-2">
          <Button
            onClick={() => {
              if (!showQBOTChat) {
                setShowQBOTChat(true);
            ...
            variant="outline"
            size="sm"
            className="bg-gradient-to-r from-orange-500 to-yellow-500 border-2 border-red-400 text-white hover:from-red-500 hover:to-orange-500 hover:border-red-500 shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 text-xs px-2 font-bold"
            title={showQBOTChat ? (isQBOTMinimized ? "Expand QBOT" : "Minimize QBOT") : "Open QBOT - Maritime Assistant"}
          >
            <i className="fas fa-robot mr-2 text-yellow-200"></i>
            <span>QBOT</span>
            {showQBOTChat ? (
              isQBOTMinimized ? (
                <ChevronUp size={14} className="ml-1 text-yellow-200" />
